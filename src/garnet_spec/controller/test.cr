  ...
  abstract class Test
    macro inherited
        ...
        {% for method in http_verbs %}
          def {{method.id}}(path, headers : HTTP::Headers? = nil, body : String? = nil, current_user = nil)
            request = HTTP::Request.new("{{method.id}}".upcase, path, headers, body )
            {% if http_write_verbs.includes? method %}
              request.headers["Content-Type"] ||= "application/x-www-form-urlencoded"
            {% end %}
            process_request(request, current_user)
          end
        {% end %}
      end

    private def process_request(request, current_user = nil)
      io = IO::Memory.new
